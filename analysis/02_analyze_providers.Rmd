---
title: "The illusion of predictability in common scientific visualizations, medical providers"
date: "`r Sys.Date()`"
...
The median time to complete the experiment was `r median(df$time_spent)` minutes. The plot below is limited to participants who completed in 100 minutes or less, excluding the top `r round(mean(df$time_spent > 100)*100 *10)/10` percent of outliers in terms of completion time.
...
We see differences of 9 and 8 percentage points for the blood pressure and COVID-19 scenarios here, with standard deviations of around 12 (when people see SDs) and 16 (when people see SEs, due to the extreme responses). Assuming the smallest difference we care about is `r min_diff` percentage points, and taking a pessimistic estimate of the standard deviation of probability of superiority estimates to be `r max_sd`, let's compute the sample size we'd need for a one-sided test with 95% power.

```{r first-superiority-estimate-power-calc}
...
To detect an actual difference of at least `r min_diff` percentage points 95% of the time we'd need approximately `r ceiling(power_calc$n)` participants in each of the four conditions in the experiment for a total of approximately `r round(4*ceiling(power_calc$n) / 25) * 25` participants. 
...
```{r difference-in-superiority-estimates}
...
# Saving variables for .rtex document
