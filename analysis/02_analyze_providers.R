...
df_raw <- read_tsv(here('raw-data/providers.tsv')) %>% distinct()

df_including_previously_attempted <- preprocess_df(df_raw) %>%
  mutate(time_spent = difftime(as.POSIXct(ts_final_submit),
                               as.POSIXct(ts_start),
                               units = "min"))

df <- df_including_previously_attempted %>%
  filter(previously_attempted != 1)



## ----filter-------------------------------------------------------------------
df <- df %>% drop_na(matches('(superiority|cost)_estimate_'))
nrow(df)
...
## ----first-superiority-estimate-power-calc------------------------------------
...
## ----difference-in-superiority-estimates--------------------------------------
...
#   left_join(true_effects) %>%
#   mutate(abs_err = abs(first_superiority_estimate - 100*auc)) %>%
#   ggplot(aes(x = abs_err, y = first_condition, color = first_condition, fill = first_condition)) +
#   geom_density_ridges(stat = "binline", binwidth = 1, scale = scale) +
#   geom_errorbarh(data = plot_data, aes(x = mu, xmin = mu - se, xmax = mu + se, y = y + scale), height = 0.1) +
#   geom_point(data = plot_data, aes(x = mu, y = y + scale)) +
#   coord_cartesian(ylim = c(1.25, 2.25)) +
#   labs(x = 'Estimated probability of superiority\nfor first visualization seen',
#        y = 'First visualization seen',
#        title = 'Distribution of probability of superiority estimates for first visualization seen') +
#   facet_wrap(~ medication_type, ncol = 1) +
#   theme(legend.position = "none")
...
## -----------------------------------------------------------------------------
ggplot(df, aes(x = log10(first_cost_estimate), y = first_condition, color = first_condition, fill = first_condition)) +
  geom_density_ridges(stat = "binline", binwidth = 1, scale = scale) +
...
providers_value_of_treatment <- ggplot(df, aes(x=first_condition_compressed, y = first_cost_estimate)) +
      geom_point(aes(color = first_condition_compressed), alpha = 0.70, position = position_jitter(width = 0.25, height = 0.15)) +
      stat_summary(fun = mean, geom = "point", size=1.2) +
      stat_summary(fun.data="mean_se",  fun.args = list(mult=1), 
                   geom="errorbar", color = "black", width=0.1) +
...
## -----------------------------------------------------------------------------
ggsave("./figures/providers-first-saw-recalled.pdf", width=4, height=4)


## ----retest-first-superiority-estimate----------------------------------------

df_recalled_correct <- df %>%
...
## ----distbuilder_histograms---------------------------------------------------

# create pretty bin labels
bin_labels <- df %>%
...
# extract the number of balls each person placed in each bin
# one entry per row
distbuilder_values_by_worker <- df %>% 
  select(worker_id, medication_type, first_condition,
         superiority_estimate_sd, superiority_estimate_se,
...
# plot distributions by first_condition and current condition
distbuilder_values_by_worker %>%
  ...
  group_by(distribution, treatment_or_control, se_or_sd, medication_type, first_condition, bin, bin_number) %>%
  summarize(total_count = sum(count)) %>%
  group_by(medication_type, first_condition, distribution) %>%
  mutate(frac = total_count / sum(total_count)) %>%
  #mutate(bin = row_number()) %>%
  ggplot(aes(x = bin, y = frac, fill = treatment_or_control)) +
    #geom_vline(data = true_dists,
    #           aes(xintercept = mean, color = distribution),
    #           linetype = 'dotted') +
    geom_bar(stat = "identity", position = "identity", alpha = 0.5) +
    facet_grid(se_or_sd ~  medication_type + first_condition, scale = "free") +
    labs(x = 'Bin',
...
# compute_bin_center <- Vectorize(function(s) { eval(parse(text=gsub('-', '+', s)))/2 })
# 
# distbuilder_values_by_worker %>%
#   group_by(worker_id) %>%
#   filter(distribution == first(distribution)) %>% 
#   mutate(bin_center = compute_bin_center(bin)) %>%
#   group_by(worker_id, first_condition, medication_type) %>%
#   do({
#     data.frame(distribution = rep(.$distribution, .$count),
#                x = rep(.$bin_center, .$count))
#   }) %>%
...
plot_data_distbuilder <- distbuilder_values_by_worker %>%
...
providers_all_first_condition_distbuilder <- ggplot(plot_data_distbuilder, aes(x = center, y = frac)) +
    #geom_vline(data = true_dists,
    #           aes(xintercept = mean, color = distribution),
    #           linetype = 'dotted') +
...
providers_all_first_condition_distbuilder


## -----------------------------------------------------------------------------
ggsave("figures/providers_all_first_condition_distbuilder_responses.pdf", width=4, height=4)
...
implied_auc_by_worker <- get_implied_auc_by_worker(distbuilder_values_by_worker)
...
## -----------------------------------------------------------------------------
plot.implied.psups <- plot_implied_auc(implied_auc_by_worker)
plot.implied.psups

## -----------------------------------------------------------------------------
ggsave("figures/providers_implied_distbuilder_psups.pdf", width=5, height=3)
...
num_provider_participants <- nrow(df)
num_provider_previous_completions <- nrow(df_including_previously_attempted) - num_provider_participants

save(
  num_provider_participants,
  num_provider_previous_completions,
  ...
  covid_extreme_se_perc,
  covid_extreme_sd_perc,
  blood_extreme_se_perc,
  blood_extreme_sd_perc,
  blood_extreme_test,
  covid_extreme_test,
  ...
  provider_recall_SE_mean,
  provider_recall_SE_mean_95_CI_lower,
  provider_recall_SE_mean_95_CI_upper,
  
  estimated_value_blood_se,
  estimated_value_blood_sd,
  estimated_value_covid_se,
  estimated_value_covid_sd,
  distbuilder.psup.est,
  distbuilder.psup.ttest,
  
  blood.median.test.pval, 
  blood.median.test.zval, 
  covid.median.test.pval, 
  covid.median.test.zval, 
  
  file="results/providers.Rdata")
